...
object PresidentialSentiment{
  def main(args: Array[String]) {
    ...
    //val sentiments = lines.map(line => get_sentiment(line, props))

    //val mapper_sentiment = tweet_stream.map(tweet => ( compact(render(tweet \ "text")), 1) )   
    ...
    val candidate_counts_sentiments = mapper_candidate_counts1.reduceByKeyAndWindow(
        (a :(Int,Int,Int,Int), b:(Int,Int,Int,Int)) => (a._1 + b._1, a._2 + b._2, a._3+b._3, a._4+b._4), Seconds(10), Seconds(10))
    val state_candidate_count = mapper_states_candidates1.reduceByKeyAndWindow(
        (a:Int, b:Int) => a+b, Seconds(10), Seconds(10)
        )
    ...
    candidate_counts_sentiments.foreachRDD(rdd => {rdd.collect.foreach(record=>{out.println(record)})})

    val server1 = new ServerSocket(8000,200);
    val conn1 = server1.accept()
    val out1 = new PrintStream(conn1.getOutputStream());
    state_candidate_count.foreachRDD(rdd => {rdd.collect.foreach(record=>{out1.println(record)})})
